...
use App\Models\User;
use App\Notifications\SasaranNotification;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Auth;

class UserRiskRegisterController extends Controller
{
    public function pilihPeriode()
    {
        $periodes = Periode::all();
        return view('user.periode', compact('periodes'));
    }

    public function submitPeriode(Request $request)
    {
        return redirect()->route('user.sasaran', $request->periode_id);
    }
    ...
    public function sasaran(Periode $periode)
    {
        ...
        $departemens = Departemen::where('id', $user->departemen_id)
            ->with(['sasarans' => function ($q) use ($periode) {
                $q->where('periode_id', $periode->id);
            }])
            ->get();

        return view('user.sasaran', compact('periode', 'departemens'));
    }
    ...
    public function createSasaran(Request $request)
    {
        ...
        */
        ...
        foreach ($admins as $admin) {
            $admin->notify(new SasaranNotification($sasaran, auth()->user()));
        }

        return redirect()
            ->route('user.sasaran', $request->periode_id)
            ->with('success', 'Sasaran berhasil ditambahkan.');
    }

    public function updateSasaran(Request $request, Sasaran $sasaran)
    {
        abort_if(
            $sasaran->departemen_id !== auth()->user()->departemen_id,
            403
        );
        ...
        return redirect()
            ->route('user.sasaran', $sasaran->periode_id)
            ->with('success', 'Sasaran berhasil diperbarui.');
    }
    ...
    public function createSebabRisiko(Request $request, Sasaran $sasaran)
    {
        ...
        return redirect()->route('user.detail', $sasaran->id)->with('success', 'Sebab risiko berhasil ditambahkan.');
    }
    ...
    public function updateSebabRisiko(Request $request, Sasaran $sasaran)
    {
        ...
        return redirect()->route('user.detail', $sasaran->id)->with('success', 'Sebab risiko berhasil diperbarui.');
    }
    ...
    public function deleteSebabRisiko(Request $request, Sasaran $sasaran)
    {
        ...
        return redirect()->route('user.detail', $sasaran->id)->with('success', 'Sebab risiko berhasil dihapus.');
    }  
    ...
    public function createPerlakuanRisiko(Request $request, SebabRisiko $sebab_risiko)
    {
        ...
        return redirect()->route('user.detail', $sebab_risiko->sasaran_id)->with('success', 'Perlakuan risiko berhasil ditambahkan.');
    }
    ...
    public function updatePerlakuanRisiko(Request $request, SebabRisiko $sebab_risiko)
    {
        ...
        return redirect()
            ->route('user.detail', $sebab_risiko->sasaran_id)
    ...
    public function deletePerlakuanRisiko(Request $request, SebabRisiko $sebab_risiko)
    {
        ...
        return redirect()->route('user.detail', $sasaranId)->with('success', 'Perlakuan risiko berhasil dihapus.');
    }
}