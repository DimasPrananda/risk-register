    ...
    <div x-data="{ open: false, openSebabId: null, showModal: false, addPerlakuanId: null, editPerlakuanId: null, editId: null, selected: null, sebab_risiko: { id: null, nama_sebab: '', pengendalian_internal: '', referensi_pengendalian: '', efektifitas_pengendalian: '', dampak: '', probabilitas: '', kategori_id: '' } }">
        ...
        <div class="ml-20 max-w-7xl mx-auto overflow-x-auto mb-4 p-1">
            ...
            <div class=" text-right my-5 flex justify-end gap-3">
                ...
                @else
                    <span class="inline-flex items-center px-4 py-2
                        bg-yellow-100 text-yellow-700 rounded-md text-sm font-semibold">
                        ⏳ Belum Publish
                    </span>
                @endif
                @if($sasaran->is_published)
            ...
            @if($sasaran->sebabRisikos->count())
                @foreach($sasaran->sebabRisikos as $item)
                            ...
                            <div class="shrink-0">
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        ...
                                        <form method="POST" action="{{ route('user.detail.destroy', $sasaran->id) }}">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <div x-show="openSebabId === {{ $item->id }}"
                            ...
                            <div class="mt-4 p-4 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-700">
                                ...
                                @foreach($item->perlakuanRisikos as $perlakuan)
                                            ...
                                            <div class="shrink-0">
                                                <x-dropdown>
                                                    ...
                                                    <x-slot name="content">
                                                        ...
                                                        <form method="POST" action="{{ route('user.perlakuan-risiko.destroy', $item->id) }}">
                                                            @csrf
                                                            @method('DELETE')
                                    ...
                                    @include('user.detail.modal-edit-perlakuan', [
                                        'perlakuan' => $perlakuan,
                                        'item' => $item
                                    ])
                                    @endforeach
                    ...
                    @include('user.detail.modal-edit-risiko', [
                        'sasaran' => $sasaran,
                        'kategoris' => $kategoris,
                        'item' => $item
                    ])
                    @include('user.detail.modal-create-perlakuan', [
                        'item' => $item
                    ])            
                @endforeach
            @else
        ...
        @include('user.detail.modal-create-risiko', [
            'sasaran' => $sasaran,
            'kategoris' => $kategoris
        ])
    </div>
